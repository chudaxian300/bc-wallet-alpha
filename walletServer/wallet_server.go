...
func (ws *WalletServer) walletByPrivatekey(w http.ResponseWriter, req *http.Request) {
	...
	switch req.Method {
	case http.MethodPost:
		var l LoadWalletRequest
		decoder := json.NewDecoder(req.Body)
		decoder.Decode(&l)
		w.Header().Add("Content-Type", "application/json")
		log.Println(*l.SenderPrivateKey)
		myWallet := wallet.LoadWallet(*l.SenderPrivateKey)
		m, _ := myWallet.MarshalJSON()
		io.WriteString(w, string(m[:]))
	default:
		w.WriteHeader(http.StatusBadRequest)
...
func (lr *LoadWalletRequest) Validate() bool {
	if lr.SenderPrivateKey == nil {
		return false
	}
	return true
}
...
func (ws *WalletServer) CreateTransaction(
	...
	case http.MethodPost:
		...
		// 交易签名
		tran := new(wallet.Transaction)
		hash := fmt.Sprintf("%x", tran.Hash())
		transaction := wallet.NewTransaction(privateKey, publicKey,
			*t.SenderBlockchainAddress, *t.RecipientBlockchainAddress, hash, value)
